...
echo "0 */6 * * * gcloud auth activate-service-account --key-file=/config/gcs-key.json && gsutil cp /config/world.zip gs://$GCS_BUCKET$GCS_BUCKET_PATH/world.zip" > /etc/cron.d/gcs-hourly-backup
